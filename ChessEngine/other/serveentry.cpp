...
void ServeEntry::sendToClient(const QString& text, QTcpSocket* socket)
{
    //判断是否开启了server
    if(!server->isListening())
        return;
...
void ServeEntry::readData(QTcpSocket* socket)
{
...
void ServeEntry::handleCmd(const QString &cmd)
{
...
bool ServeEntry::buildPos(const UcciComm &comm)
{
